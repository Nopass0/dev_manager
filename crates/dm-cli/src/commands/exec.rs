...
use crate::commands::{load_project_config, ExecArgs};
...
pub async fn run(args: ExecArgs) -> DmResult<()> {
    ...
    print_system(&format!("exec в '{}/': {}", args.service, args.command.join(" ")));
    ...
    let status = cmd.status().map_err(|e| {
        dm_core::DmError::Process(format!("не удалось запустить команду: {e}"))
    })?;
    if !status.success() {
